    ...
    modal.show(`
      <div class="modal-header">
        ...
        <div style="display:flex;gap:6px">
          <button class="btn-icon" title="Baixar imagem do Last.fm" onclick="downloadArtistaCover('${a.id}')">
          ...
          <button class="btn-icon" title="Trocar imagem por URL" onclick="changeArtistaImage('${a.id}')">
...
async function downloadArtistaCover(id) {
  try {
    toast('Buscando imagem no Last.fm...');
    await api.downloadArtistaImage(id);
    ...
    if (msg.includes('404') || /não encontrad/i.test(msg)) {
      toast('Last.fm não tem imagem pra esse artista', 'error');
    } else {
